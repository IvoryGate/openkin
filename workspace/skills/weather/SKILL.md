...
skill-id: weather
description: |
  Use when: 用户明确要「天气 / 气温 / forecast」且已给出或可推断城市名。
  Don't use when: 未给出城市且无法合理默认；或用户只是在比喻「心情天气」等非字面天气。
  反例：问「今天适合出门吗」但未提地点——应先澄清城市再用本 skill。
  查询城市天气预报（模拟数据）。输入 city（城市名，支持中文或英文），返回天气预报字符串。
permissions:
  read: ["."]
  net: []
  write: []

...
账号状态机继续沿用 `docs/CHANNELS.md` 已存在的首期状态：
...
| 文档 | 更新 `docs/CHANNELS.md`，明确 framework 与具体平台的边界 |
...
- 更新 `docs/CHANNELS.md`，明确首期只做 framework，不做平台实现
...
3. `docs/CHANNELS.md` 明确说明 framework 边界、状态机冻结与后续平台接入方式。
4. `pnpm verify` 通过。

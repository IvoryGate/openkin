...
| 文档                          | 更新 `docs/SDK.md`，明确 implemented / deferred 能力面 |
...
- 更新 `docs/SDK.md`，把首期能力划分为 `implemented` 与 `deferred`
...
2. `docs/SDK.md` 与实际实现一致，并明确列出 deferred 能力。
3. `pnpm verify` 通过。
4. `pnpm test:sdk` 通过。
...
- **日期**：2026-04-02
- **实现**：`packages/sdk/client`（`createOpenKinClient`、`parseSseStreamEvents`）；`scripts/test-sdk.mjs` + `scripts/run-sdk-smoke.ts`（真实 server 子进程 + E2E）；`docs/SDK.md` 已区分已实现/延后。

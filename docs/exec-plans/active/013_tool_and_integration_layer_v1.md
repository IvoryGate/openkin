...
- 验证由 `scripts/test-tools.mjs`（或等价 smoke 脚本）完成。
...
| `scripts/` | 新增 `test-tools.mjs`（smoke 脚本）或等价入口 |
| `package.json`（根） | 新增 `test:tools` 脚本，纳入 `verify` 链 |
| 文档 | 更新 `docs/architecture/ARCHITECTURE.md`（Tool Layer 首期状态说明） |
...
- `packages/sdk/client/`（不需要 SDK 层变化）
...
3. **新建** `packages/core/src/tools/index.ts`
   - 导出 `echoToolDefinition`、`echoToolExecutor`、`getCurrentTimeToolDefinition`、`getCurrentTimeToolExecutor`
...
- 不实现 MCP ToolProvider（另开执行计划）
- 不实现 Skill ToolProvider（另开执行计划）
- 不迁移 `demo-shared.ts` 的 `get_weather`（它是 demo harness，不是生产工具）
- 不新增 Tool Layer 的服务级 API（如工具注册、工具列表查询 endpoint）
...
- **不依赖** 004/005/006（Service / SDK / Channel 层已稳定，本计划不改动这些层）
- **后续候选**：
  - `014` — MCP ToolProvider 接入（接一个真实 MCP server，验证 `ToolProvider` 抽象扩展性）
  - `015` — Skill ToolProvider（按 `sourceType: 'skill'` 语义实现 skill 加载框架）
  - 或 Service 层深化（鉴权、CORS、错误中间件）
  - 或 Channel 层首个真实平台接入

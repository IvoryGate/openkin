...
**首期方案（简单可验证）**：
- 用户在群内发消息 → 所有群内 Agent 依次收到 run（串行，间隔 500ms）
- 每个 Agent 的回复作为该 Session 的 assistant message 出现
...
| 工单 | 内容 | 预估 |
|------|------|------|
| 153-B1 | 群组数据模型 + 创建群聊 UI | 1d |
| 153-B2 | 群聊消息路由（多 Agent 串行回复） | 1.5d |
| 153-B3 | 群聊消息气泡（区分不同 Agent + 头像+名称标识） | 1d |
| 153-B4 | 群成员管理面板（添加/移除/解散） | 1d |

**Wave B 完成后验收**：可以新建群聊，选择多个 Agent，发消息后所有 Agent 依次回复，每个 Agent 的回复都有清晰的来源标识。
...
群聊在客户端侧映射为一个 `Session`（kind=channel），群内所有 Agent 共享同一个 Session 的消息历史。用户发消息时，客户端依次为每个 Agent 创建 Run（同一 Session），Agent 回复作为 assistant message 出现。

优点：
- 不需要新增后端 API
- 所有 Agent 共享上下文，能看到彼此的回复
- 简单直接，首期可快速验证

缺点：
- 串行回复延迟较高（后续可改为并行）
- 所有 Agent 共享 token 预算（长群聊可能溢出）
- 无真正的"群组"持久化在后端

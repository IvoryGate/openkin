...
- 当前说明：统一 `pnpm verify`（scenarios + **第一层审计** `test:first-layer-audit` + server + sdk + channels）；dev-console mock / live demo；非默认真实 provider 验收：`pnpm test:first-layer-real`（见 `../first-layer/DEMO_FIRST_LAYER.md`）

...
- `pnpm test:first-layer-real`（单次 `demo:live` 冒烟；需 `OPENAI_*`；见 `docs/DEMO_FIRST_LAYER.md`）
- `pnpm test:first-layer-real-audit`（第一层全链路真实 API 审计：工具、记忆、多轮、压缩、hook 中止；见 `docs/FIRST_LAYER_COVERAGE.md`）

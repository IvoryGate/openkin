...
#### 2.3.6 可插拔向量存储（`server/vector-store`）

长期记忆和知识库都依赖向量检索。第一阶段先用 SQLite 自带方案（`sqlite-vec` 扩展或纯 FTS5），后续可无缝升级到独立向量数据库。

**接口抽象：**

```typescript
interface VectorStore {
  insert(id: string, vector: number[], metadata: object): Promise<void>
...
| 方案                           | 适用阶段       | 说明                                            |
| ------------------------------ | -------------- | ----------------------------------------------- |
| **SQLite + FTS5**              | 第一阶段（P0） | 全文检索，无真正向量搜索，仅用于原型验证        |
| **sqlite-vec**                 | 第二阶段（P1） | SQLite 官方向量扩展，支持余弦相似度，单文件部署 |
| **Qdrant / Milvus / Weaviate** | 第三阶段（P2） | 生产级向量数据库，支持混合检索、过滤、分片      |

- [ ] 定义 `VectorStore` 接口
- [ ] 实现 `SQLiteFTS5Store`（纯关键词检索，用于第一阶段快速验证）
- [ ] 实现 `SqliteVecStore`（集成 `sqlite-vec` 扩展）
- [ ] 提供 `VectorStoreFactory`，按配置切换实现
- [ ] LongTermMemory 和 KnowledgeBase 通过工厂获取 VectorStore 实例
...
> 详细设计见 [`docs/具体模块扩展介绍/Scheduled_Task_System.md`](./具体模块扩展介绍/Scheduled_Task_System.md)

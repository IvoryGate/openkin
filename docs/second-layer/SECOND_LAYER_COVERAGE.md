...
| `inline` 模式 | **不开放**（待 017） | 需要 Deno 沙箱后启用 |
...
| 能力 | `test:sandbox` | 说明 |
|------|---------------|------|
| 正常 Skill 在 Deno 沙箱下正常执行 | ✓ | 结果与无沙箱一致 |
| 读取 skills/ 外文件被拒绝 | ✓ | Deno 拒绝，exitCode 非 0 |
| 未声明网络连接被拒绝 | ✓ | Deno 拒绝 |
| `inline` 合法代码执行 | ✓ | 纯计算逻辑，无 I/O |
| `inline` 读文件被拒绝 | ✓ | 最严格权限，无 --allow-read |
| `inline` 临时文件执行后删除 | ✓ | 断言临时文件不存在 |
| `permissions` 缺失使用默认权限 | ✓ | 兼容性验证 |
| `net: ["*"]` 被框架拒绝 | ✓ | 配置错误在执行前拦截 |
| Deno 不可用时降级到 tsx | skip（Deno 可用时不测降级路径） | 降级由实现逻辑保证 |
...
该命令在 013–016 稳定后另开计划添加，不并入 `pnpm verify`。

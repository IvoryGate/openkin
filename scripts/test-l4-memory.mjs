/**
 * L4 102: layered memory taxonomy + `inspect memory` (same L3 GET as 101).
 */
...
function runCliInspectMemory(base, traceId) {
  const r = spawnSync(
    process.execPath,
    [
      '--import=tsx/esm',
...
function runCliInspectMemoryTaxonomy(base) {
  const r = spawnSync(
    process.execPath,
    ['--import=tsx/esm', cli, 'inspect', 'memory', '--server-url', base],
    { encoding: 'utf8', cwd: root, maxBuffer: 5 * 1024 * 1024, env: { ...process.env, THEWORLD_API_KEY: '' } },
  )
  return `${r.stdout ?? ''}${r.stderr ?? ''}`
}
...
async function main() {
  const docPath = join(root, 'docs/architecture-docs-for-agent/fourth-layer/L4_LAYERED_MEMORY.md')
  const doc = readFileSync(docPath, 'utf8')
  assert(doc.includes('102'), 'L4_LAYERED_MEMORY.md should reference 102')

  const port = await getFreePort()
  const tmpBase = mkdtempSync(join(tmpdir(), 'theworld-l4mem-'))
  ...
  try {
    ...
    const tax = runCliInspectMemoryTaxonomy(base)
    for (const needle of ['L4 layered memory', 'Postponed', 'theworld inspect memory <traceId>']) {
      assert(tax.includes(needle), `taxonomy output missing: ${needle}\n${tax.slice(0, 2000)}`)
    }
    ...
    const out = runCliInspectMemory(base, traceId)
    for (const needle of [
      'traceId:',
      'Step 0',
      'L3 memory contributions',
      'theworld inspect context',
    ]) {
    ...
    assert(slashSrc.includes("head === '/memory'"), 'slash /memory should exist')

    const mapSrc = readFileSync(join(root, 'packages/cli/src/l4-product-map.ts'), 'utf8')
    assert(mapSrc.includes('inspect:memory'), 'l4-product-map: inspect:memory')
    assert(mapSrc.includes('slash:/memory'), 'l4-product-map: slash:/memory')

    console.log('test:l4-memory passed')
  } finally {
    child.kill('SIGTERM')
